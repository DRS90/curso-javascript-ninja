(function () {
  ...
  */
  ...
  brasil.unshift('Pará', 'Acre', 'Roraima')
  ...
  */
  ...
  function filtraRegiao(regiao) {
    return function (pais) {
      return regiao.filter(estado =>
        pais.filter(regiaoPais =>
          regiaoPais === estado
        )
      )
    }
  ...
  */
  ...
  var isBiggerThan7 = (array => array.every(item => item.length > 7))

  console.log( isBiggerThan7(brasil) ? 'Sim, todos os estados tem mais de 7 letras!' : 'Nem todos os estados tem mais de 7 letras!' )
  ...
  */
  ...
  var hasCeara = (array => array.some(item => item === 'Ceará'))

  hasCeara(brasil) ? console.log('Ceará está incluído!') : console.log('Ceará não foi incluído :(')
  ...
  */
  var filter = map.filter(item => !(item.id % 2))
